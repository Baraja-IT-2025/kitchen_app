...
class _KitchenDashboardState extends State<KitchenDashboard>
  ...
  Future<void> confirmOrderToServer(String orderCode) async {
    final uri = Uri.parse('http://10.0.2.2:3000/api/confirm-order/$orderCode');
    try {
      final response = await http.post(uri);
      if (response.statusCode == 200) {
        print('Order $orderCode confirmed successfully.');
      } else {
        print('Failed to confirm order $orderCode: ${response.body}');
      }
    } catch (e) {
  ...
  void _confirmOrder(Order order) async {
    ...
    await confirmOrderToServer("ORD-19E05-005");
    setState(() {
      orders.remove(order);
...
class Order {
  ...
  String get timeSinceOrderText {
    ...
    return "Waktu konfirmasi: ${minutes}:${remainingSeconds.toString().padLeft(2, '0')}";
  }
